...
class StatsView @JvmOverloads constructor(
    ...
    init {
        context.withStyledAttributes(attrs, R.styleable.StatsView) {
            val test = getInteger(R.styleable.StatsView_animationType, animationType)
            animationType = getInteger(R.styleable.StatsView_animationType, animationType)
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        when (animationType) {
            0 -> {
                var startFrom = -90F
                val sum = data.sum()
                val max = 360 * progress
                for ((index, datum) in data.withIndex()) {
                    val angle = min(360F * (datum / sum), max)
            ...
            1 -> {
                ...
                for ((index, datum) in data.withIndex()) {
                    ...
                    paint.color = colors.getOrNull(index) ?: randomColor()
                    canvas.drawArc(oval, startFrom + angle / 2, angle * progress, false, paint)
                    canvas.drawArc(oval, startFrom + angle / 2, angle * progress * -1, false, paint)
                    startFrom += angle
                }
            }
        }
